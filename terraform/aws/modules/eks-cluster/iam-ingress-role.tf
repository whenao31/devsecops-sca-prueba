    ...
    "Statement": [
      {
        ...
        "Condition": {
          "StringEquals": {
            ...
            format("%s:sub", join("/", slice(split("/", aws_iam_openid_connect_provider.eks.id), 1, length(split("/", aws_iam_openid_connect_provider.eks.id))))): "system:serviceaccount:kube-system:aws-load-balancer-controller"
          }
        }
      },
    ]
...
resource "aws_iam_role_policy" "ingress_role_policy" {
